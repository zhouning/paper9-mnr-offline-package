...
地类编码按用户提供的“一张图”标准所引用的 `GB/T 21010-2017` / 第三次全国国土调查体系
识别。现行耕地基码为 `0101/0102/0103`，林地为 `0301` 至 `0307`，园地为 `0201` 至
`0204`。最多五位的原始码完整保留，分类使用前四位基码。旧三位测试码可单独兼容，但与
现行四位耕地/林地码混用、无法识别耕地或无法识别林地都会使融合失败。
...
DLTB 行政区码和完整路径建立不带前导连字符的县级隔离目录。容器退出后，wrapper 在宿主机
验证六个融合文件，生成 `DATA_ROOT/FUSION_OUTPUTS.txt`，然后打印成果绝对路径和后续四条
完整命令。
